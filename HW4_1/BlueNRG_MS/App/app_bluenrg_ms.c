...
volatile uint8_t g_is_connected = 0;          // Flag to indicate BLE connection status
volatile uint16_t g_sampling_period_ms = 1000; // Default sampling period: 1000ms (1Hz)

// --- Custom BLE Service Handles ---
uint16_t Acc_Service_Handle;      // Handle for the main service
uint16_t Acc_Data_Char_Handle;    // Handle for the 3-axis data characteristic (Notify)
uint16_t Acc_Freq_Char_Handle;    // Handle for the frequency characteristic (Write)

// --- Custom 128-bit UUIDs ---
// Use an online generator to create your own unique UUIDs
const uint8_t ACC_SERVICE_UUID[16] =
    {0x42, 0x72, 0x6E, 0x64, 0x47, 0x48, 0x4D, 0x73, 0x41, 0x64, 0x6D, 0x49, 0xC0, 0x00, 0x00, 0x00};
const uint8_t ACC_DATA_CHAR_UUID[16] =
    {0x42, 0x72, 0x6E, 0x64, 0x47, 0x48, 0x4D, 0x73, 0x41, 0x64, 0x6D, 0x49, 0xC1, 0x00, 0x00, 0x00};
const uint8_t ACC_FREQ_CHAR_UUID[16] =
    {0x42, 0x72, 0x6E, 0x64, 0x47, 0x48, 0x4D, 0x73, 0x41, 0x64, 0x6D, 0x49, 0xC2, 0x00, 0x00, 0x00};
/* USER CODE END PV */
...
/* USER CODE BEGIN PFP */
tBleStatus Add_Accelerator_Service(void);
/* USER CODE END PFP */
...
void MX_BlueNRG_MS_Init(void)
{
  ...
  // --- ADD THIS CALL BACK ---
    ret = Add_Accelerator_Service();
    if (ret != BLE_STATUS_SUCCESS)
    {
      PRINTF("!!! Error adding Custom Accelerator service: 0x%02X !!!\n", ret);
      Error_Handler(); // Halt on failure
    }
    // --------------------------
...
/* PASTE THESE THREE FUNCTIONS AT THE VERY END OF app_bluenrg_ms.c */

/**
 * @brief  Add the custom Accelerator service and its characteristics.
 * @param  None
 * @retval Status
 */
tBleStatus Add_Accelerator_Service(void)
{
  ...
  // 1. Add the Service
  memcpy(uuid, ACC_SERVICE_UUID, 16);
  ret = aci_gatt_add_serv(UUID_TYPE_128,         // 128-bit UUID
                          uuid,                  // Service UUID buffer
                          PRIMARY_SERVICE,       // Service Type
                          7,                     // Max Attribute Records needed
                          &Acc_Service_Handle);  // Output handle
  if (ret != BLE_STATUS_SUCCESS) {
    PRINTF("Error adding Acc Service: 0x%02X\n", ret);
    return ret;
  }

  // 2. Add the Accelerator Data Characteristic (Notify)
  memcpy(uuid, ACC_DATA_CHAR_UUID, 16);
  ret = aci_gatt_add_char(Acc_Service_Handle,      // Service handle
                          UUID_TYPE_128,         // 128-bit UUID
                          uuid,                  // Characteristic UUID buffer
                          6,                     // Max Length (3 axes * 2 bytes/axis)
                          CHAR_PROP_NOTIFY,      // Properties (Notify)
                          ATTR_PERMISSION_NONE,  // No security
                          0,                     // GATT event mask (no specific server events needed)
                          16,                    // Encryption key size (default)
                          1,                     // Is variable length (yes, though we always send 6 bytes)
                          &Acc_Data_Char_Handle); // Output handle
  if (ret != BLE_STATUS_SUCCESS) {
    PRINTF("Error adding Acc Data Char: 0x%02X\n", ret);
    return ret;
  }

  // 3. Add the Accelerator Frequency Characteristic (Write/Read)
  memcpy(uuid, ACC_FREQ_CHAR_UUID, 16);
  ret = aci_gatt_add_char(Acc_Service_Handle,      // Service handle
                          UUID_TYPE_128,         // 128-bit UUID
                          uuid,                  // Characteristic UUID buffer
                          2,                     // Max Length (uint16_t for period_ms)
                          CHAR_PROP_WRITE | CHAR_PROP_READ, // Properties (Write + Read)
                          ATTR_PERMISSION_NONE,  // No security
                          GATT_NOTIFY_ATTRIBUTE_WRITE, // Event mask: notify app on write
                          16,                    // Encryption key size (default)
                          0,                     // Is variable length (no, always 2 bytes)
                          &Acc_Freq_Char_Handle); // Output handle
  if (ret != BLE_STATUS_SUCCESS) {
    PRINTF("Error adding Acc Freq Char: 0x%02X\n", ret);
    return ret;
  }

  PRINTF("Custom Accelerator Service Added OK: ServiceH %d, DataH %d, FreqH %d\n", Acc_Service_Handle, Acc_Data_Char_Handle, Acc_Freq_Char_Handle);
  return BLE_STATUS_SUCCESS; // Use BLE_STATUS_SUCCESS explicitly
}


/**
 * @brief  FreeRTOS Task for handling BLE events.
 * @param  argument: Not used
 * @retval None
 */
void TASK_BLE(void *argument)
{
  PRINTF("--- TASK_BLE Started ---\n");
  (void)argument;
  for(;;)
  {
...
/**
 * @brief  FreeRTOS Task for reading accelerator and notifying BLE client.
 * @param  argument: Not used
 * @retval None
 */
void TASK_ACC(void *argument)
{
  ...
  for(;;)
  {
    if (g_is_connected)
    {
      ...
      aNotifyBuffer[3] = (uint8_t)((acc_data[1] >> 8) & 0x00FF);
